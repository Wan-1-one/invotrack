            ...
            <div class="border-b pb-6 mb-6">
                <div class="flex justify-between items-start">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Order: {{ $invoice->order ? $invoice->order->order_number : 'N/A' }}</p>
                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <span class="font-medium">{{ $invoice->order ? $invoice->order->order_number : 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Order Date:</span>
                            <span class="font-medium">{{ $invoice->order ? $invoice->order->created_at->format('M d, Y') : 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Order Status:</span>
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                {{ $invoice->order && $invoice->order->status === 'confirmed' ? 'bg-green-100 text-green-800' : 
                                   ($invoice->order && $invoice->order->status === 'pending' ? 'bg-yellow-100 text-yellow-800' : 'bg-gray-100 text-gray-800') }}">
                                {{ $invoice->order ? $invoice->order->status : 'N/A' }}
                            </span>
                        </div>
                        <div class="flex justify-between">
                            <span class="text-gray-600">Customer:</span>
                            <span class="font-medium">{{ $invoice->order ? $invoice->order->customer_name : 'N/A' }}</span>
                        </div>
                    </div>
                </div>
            </div>
